...
![Architecture diagram showing the Identity & Policy Middleware as the only enforcement point between an Agent's disposable container and the Mock Resource Service, with both the allow and deny branches writing a trace span](docs/assets/architecture-diagram.png)

The middleware is the only path to a protected resource. The decision is made outside the Agent's control, on the trusted side of the boundary. A denied request never reaches the resource service. Every branch, allowed or denied, writes a span tagged with the run's `runId`.
...
- Per-Agent identity with scoped tokens — no more shared demo credential
- A single backend enforcement point that authorizes every resource fetch
- Live revocation and reissue: kill a token mid-run, then restore access with a fresh one
- A `runId`-correlated trace with a UI timeline showing every allow/deny decision and its reason
...
- Python 3.10+ (for the mock resource service)
- A Volcengine Ark API key and endpoint that supports the Responses API

Codex CLI is included in the Runtime image and is not required on the host.

The middleware itself — scoped fetch, denial, revocation, and the trace — is
fully observable without a working Ark endpoint: `/api/resource/fetch` never
calls the model. Only the Agent-driven prompt-injection beat (Demo Walkthrough
beat 2) needs a working `ARK_API_KEY` and `ARK_MODEL`.
...
The Agent-scoped-access demo (Demo Walkthrough beat 2) needs a protected
resource to fetch from. In a separate terminal:
...
| `APP_AUTH_TOKEN` | Auto-generated by `npm run poc` | The human operator's control-plane credential, separate from any Agent's own token. Set it yourself for a stable demo value; use 24+ random characters remotely. |
...
```mermaid
flowchart LR
    Operator["Human operator<br/>(bearer token)"] --> UI["React Web UI"]
    UI --> API["Fastify control plane"]
    API --> Store["JSON store<br/>(agents, tokens, spans)"]
    API --> Runtime{"Runtime provider"}
    Runtime -->|Local POC| Container
    Runtime -->|ECS profile| Codex["Codex CLI in application container"]

    subgraph Boundary["Trust boundary"]
        Container["Disposable container<br/>(holds only its X-Agent-Token)"]
    end

    Container -->|only route it can reach| Enforce["Identity & Policy Middleware"]
    Enforce -->|deny: unknown_token, revoked, or out_of_scope| Store
    Enforce -->|allow| Mock["Mock Resource Service"]
    Mock --> Store
...
See the [Architecture](#architecture) diagram above for how a denied request is stopped before reaching the resource.
