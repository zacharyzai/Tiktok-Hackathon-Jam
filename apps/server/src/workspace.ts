...
export class WorkspaceManager {
  ...
  async writeInstructions(agent: Agent): Promise<void> {
    const content = [
      ...
      "## Protected resources",
      "",
      "To read a protected resource, run:",
      "  curl -s -X POST $AGENT_RESOURCE_ENDPOINT \\",
      '    -H "Content-Type: application/json" -H "X-Agent-Token: $AGENT_TOKEN" \\',
      '    -d \'{"resource":"<owner>/<name>","runId":"\'"$AGENT_RUN_ID"\'"}\'',
      "Only resources your token's scopes allow will be returned; others are denied.",
      "",
